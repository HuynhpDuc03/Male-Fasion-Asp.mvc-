...
namespace MaleFashion.Controllers
{
    public class ShopController : Controller
    {
        ...
        {
            var products = dB.SanPham_KichThuoc_MauSac.FirstOrDefault(x => x.MaSP == id);
            var kichThuoc = dB.SanPham_KichThuoc_MauSac.Where(x => x.SanPham.MaSP == id).Select(x => x.KichThuoc).Distinct().ToList();
            var mauSac = dB.SanPham_KichThuoc_MauSac.Where(x => x.SanPham.MaSP == id).Select(x => x.MauSac).Distinct().ToList();
            ...
            var relatedProducts = dB.SanPhams.Where(x => x.MaTL == products.SanPham.MaTL && x.MaSP != products.MaSP)
